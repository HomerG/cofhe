...
add(const vector<vector<T>> & a, const vector<T> & b)
{
...
add(const vector<vector<vector<vector<T>>>> & a, const vector<T> & b)
{
    auto nItems = a.size();
    auto nRows = a[0].size();
...
crtDecode(const CRT<T> & a)
{
    return a.decode();
}

template <class T> vector<vector<T>>
crtDecode(const vector<CRT<T>> & a)
{
    vector<vector<T>> r;
    for ( auto & e : a ) r.push_back( crtDecode(e) );
    return r;
}

template <class T> vector<vector<vector<T>>>
crtDecode(const vector<vector<CRT<T>>> & a)
{
...
template <class T> vector<vector<vector<vector<T>>>>
crtDecode(const vector<vector<vector<CRT<T>>>> & a)
{
...
crtDecode(const vector<vector<vector<vector<CRT<T>>>>> & a)
{
...
mean(const vector<T> & a)
{
    ...
    return add_vector(r);
    // T mean = 0;
    // size_t divider = a.size();
    // for ( const auto & e : a ) mean += e;
    // mean /= divider;
    // return mean;
}
...
{
    auto & o = order;
    if ( o.size() != 2 ) throw "order must be of size 4";
...
{
    auto & o = order;
    if ( o.size() != 3 ) throw "order must be of size 4";
