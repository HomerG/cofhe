...
file_ntt = open("./testcases/constmul.v", "w")
file_ntt.write ("uartm_write_128     (.addr(GPCFG_N_ADDR[0]),        .data(" + str(DWIDTH) + "'d"  + str(modulus) + ")); \n")
file_ntt.write ("uartm_write_128     (.addr(GPCFG_NINV_ADDR[0]),     .data(" + str(DWIDTH) + "'d"  + str(seq2[0]) + ")); \n")
...
file_ntt.write ("uartm_write         (.addr(GPCFG_FHECTLP_ADDR),  .data(32'd4));\n")


for i in range(POLYDEG):
  expctd_res[i] = seq[i] * seq2[0] % modulus
